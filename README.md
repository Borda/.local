...
Benchmark methodology and current provider-parity evidence live in [`benchmarks/README.md`](benchmarks/README.md). The executable edit suites report complete task scopes, unfavorable cells, and independent patch/oracle outcomes; they do not turn a single favorable run into a universal token-saving claim. Current Fix-Multi evidence closes P1.3 as a bounded harness/heterogeneous-evidence milestone: FM-02 and valid revised FM-03 A/C pairs are retained, incomplete FM-01 A/C pairs remain visible, and production guidance is adaptive after the invalid `Strategy.setup` contract was replaced with cooperative `Strategy.setup_environment` propagation.
