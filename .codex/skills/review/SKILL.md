...
- `commit`: review a commit or range.
...
04. T2 specialist fan-out. For `LOCAL`, `BROAD`, and `HIGH_RISK` diffs, get specialist review before final severity classification.
...
8. Missing `review-notes.md` sections => fail.

## Quality Gates

Required checks:

- `review`: T0 files, risk tier, changed-file inspection, specialist notes, severity map, and `git diff --check`.
...
- behavioral cases: false blocker, missing specialist pass, no-finding residual risk, substituted fan-out confidence
