...
Codex-native skills are portable local workflows. Runnable contract: shared result schema and local `.codex/` layout.
...
- Use `.codex/skills/_shared/collect-pr.sh --checkout` for PR diff, metadata, comments/reviews/threads, unresolved threads, target/PR refresh where possible, local checkout evidence; no ad hoc `gh`/raw URL snapshots.
- Use `.codex/skills/_shared/find-review-report.py` for path-free PR report lookup; no ad hoc JSON parsing in instructions.
- Delegation/in-main substitute passes use `.codex/skills/_shared/specialist-orchestration.md`, narrow context packs, explicit output contracts, parent consolidation.
...
- Use `.codex/skills/_shared/validate-artifacts.py` for common shapes when durable notes, ledgers, JSONL exist.
