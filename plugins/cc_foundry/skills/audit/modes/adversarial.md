...
Resolve the checklist path first — the spawned curator runs in a fresh shell and cannot expand orchestrator variables, so the resolved value is interpolated as a literal string into the prompt:
...
Use `BATCH_SIZE_MIN` (not `ADVERSARIAL_BATCH_SIZE`) for Phase D — bin/ scripts are shorter and security checks deterministic; BATCH_SIZE_MIN fine here. Plugin with ≤5 bin/ scripts: one foundry:qa-specialist spawn covers all; larger sets batch by 5.
