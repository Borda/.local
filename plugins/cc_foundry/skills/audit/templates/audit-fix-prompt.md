...
Run `cat "$AUDIT_TPL/fix-prompt.md"` via the Bash tool for the per-file fix prompt template.
...
**Model tiering (gate + fix spawns)**: tier on whether the task PRODUCES a judgment or merely APPLIES one. `haiku` (or `codex:codex-rescue`) for: applying a fix whose exact replacement wording the gate already supplied; confirming mechanical facts (grep counts, numeric equality, file existence, table-row presence). `sonnet` for fixes needing judgment about placement or phrasing. `opus` for adversarial gates on CRITICAL/HIGH findings, and any task needing empirical probing or cross-file authority determination. Tell: once a gate returns a verbatim replacement string, every downstream spawn is transcription — do not spend a reasoning tier on it. Counter-tell: a gate that must DERIVE the correct wording (from git history, runtime behaviour, or conflicting sources) earns `opus` regardless of how small the resulting diff is.
**Adversarial pre-apply gate**: for each unique file in findings list, spawn **foundry:challenger** AND **foundry:curator** in parallel — challenge/validate each finding batch: "Is each finding real? Is the fix appropriate? Does any fix risk removing load-bearing behavioral or structural/display content (e.g., CommonMark fence nesting, template rendering, visual output structure)?" Each writes verdict to `<RUN_DIR>/gate-<file-basename>.md`; return `{"verdict":"approved"|"blocked","reason":"<one-line>","file":"<path>"}`. If either returns `blocked`: mark findings for that file as blocked (add to `blocked_findings` list with reason); skip fix agent. Proceed to fix agent only if both return `approved`.
...
For each file that passed gate, spawn one fix agent (foundry:curator for .md files, foundry:sw-engineer for .js/.py files) with all approved findings batched into a single prompt.
