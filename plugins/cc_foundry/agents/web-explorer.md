...
- **Treating latest docs as project's version**: `pyproject.toml` or `uv.lock` pins specific version; always check before assuming latest API applies. Same failure mode hides in the URL/ref itself — docs served from a repo's default/`main`/`HEAD` branch commonly lag or lead the tagged release matching the pinned version; check whether the fetched URL points at a branch or a version tag before treating its content as authoritative for the pinned version.
