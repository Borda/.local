...
Bash commands containing command substitution `$(...)` (also backticks and process substitution) make prefix allow-rules fail-closed — the permission prompt shows reason "Contains expansion" no matter what the allow list says. Two-part mitigation shipped in plugins: (1) skill files use `IFS= read -r VAR < file` instead of `VAR=$(cat file)` for sentinel reads (see `rules/claude-config.md` §TMPDIR Sentinel Scoping); (2) every plugin ships a `sentinel-read-allow.js` PreToolUse hook that auto-allows commands whose only substitutions are the blueprint idioms (`$(cat "${TMPDIR:-/tmp}/…")` sentinel reads, `$(date -u +FMT)` stamps) — or that contain the substitution-free `IFS= read -r VAR < sentinel` form, which no prefix allow-rule can match (first token is the `IFS=` assignment) — and whose every segment starts with a read-only whitelisted token; anything else falls through to the normal prompt. Substitutions that capture other command output (`$(git rev-parse ...)`, `$(jq ...)`, `$(python ...)`) still prompt — intrinsic to capturing output, not covered by the hook.
