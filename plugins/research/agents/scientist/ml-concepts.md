...
Attention: self/cross/sparse/Flash; Norm: BatchNorm vs LayerNorm vs RMSNorm; Scaling — Chinchilla optimal (Hoffmann et al. 2022): for fixed pre-training compute budget C, compute-optimal model size scales as N ∝ C^0.5 (more tokens on smaller model beats fewer tokens on larger). Scope: pre-training from scratch ONLY — NOT fine-tuning or inference-time compute allocation. For capability-maximizing (not compute-optimal) training, Chinchilla point is floor not ceiling — longer training past it still improves downstream tasks (LLaMA/Mistral). Transfer: pretraining objectives, fine-tuning, prompt tuning; Uncertainty: ensembles, MC Dropout, conformal prediction.
