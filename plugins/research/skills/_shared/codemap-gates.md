...
Read `CODEMAP_CURRENCY=$(cat "${TMPDIR:-/tmp}/research-codemap-currency" 2>/dev/null || echo "no_index")`.
...
- (a) Rebuild now — `Skill(skill="codemap:scan-codebase")` then continue with fresh index (note: ambient hook may have already started background refresh; `scan-codebase` blocked by scan lockfile until it completes — up to 10 min)
