...
2. **Re-parse program file**: if `program_file` non-null, re-read/re-parse (R1 rules), update config — applies edits made between runs. Edits during active loop take effect only on next `resume`.
