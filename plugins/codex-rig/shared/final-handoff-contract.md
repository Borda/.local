...
New review-findings and remediation tables use optional `layout=grouped`. The exact machine columns, row identities, source coverage and detail bindings remain unchanged; rendering uses a short overview followed by named per-finding detail blocks. Review rows carry their canonical `title`, `summary`, and `closure_evidence`; the shared validator binds these and action/evidence cells to the enriched review record. Operational blockers use their ID as title and omit summary/closure. Remediation O/E symbols are expanded into their exact bound text. Other skills and PR Snapshot do not support grouped layout. Omitted layout preserves historical rendering bytes; never retroactively rewrite validated handoffs.

Before remediation scope selection, `final_handoff.py selection` validates `selection.json` and renders `resolution-scope.md`; `--check` verifies exact existing bytes without writes. The inventory contains `schema_version=1`, `selected_indexes=null` (pending) or the confirmed numeric list, and ordered items with `input_item_id`, `item_name`, `severity`, `selectable`, `summary`, `closure_evidence`, and full sources. Repeated report views belong in source `related_mentions`, with `finding_id` when known; they are not independent sources. Duplicate item/source/canonical-finding identities or incorrect declared counts fail before any prompt. Record `resolution_scope.presentation_version=2`; final validation binds confirmed selection and source inventory to the outcome ledger. Legacy selection artifacts retain their old validator branch.
