...
Inspect `python PLUGIN_ROOT/shared/final_handoff.py selection --help`, then run its `selection` action with `--input <run-directory>/selection.json --out-scope <run-directory>/resolution-scope.md`. Failure blocks the prompt and edits. The helper validates unique item/source/canonical-finding ownership and any declared totals before writing. It renders `# | Severity | Finding`, followed by one named group per selectable finding with `Issue`, `Done when`, and every genuine evidence reference. Related mentions are labeled separately. Long headings and S/C symbol-only cells are historical output, not the new selection format.

Before selection, say `Awaiting selection`; never imply pending findings were deliberately deferred. After explicit choice, update `selected_indexes`, rerun the helper, and record `CODE_REMEDIATE_METADATA.resolution_scope.presentation_version=2`. Final validation checks exact rendered bytes, confirmed/deferred indexes and unchanged item/source inventory. Preserve legacy scope validation for historical runs without this marker.
...
3. The complete short selection table, followed by a visually separated named detail group for every selectable item. Do not abbreviate issue, closure evidence or genuine source references.
...
60. Presentation loses or changes a required source pointer, detail, or expanded source record => fail: `code-remediate-source-presentation-mismatch`. Apply the format-specific contract: grouped selection/final output retains ordered pointers and complete labeled details in each named finding block, without requiring visible symbols; durable ledgers and historical symbol layouts retain compact source cells and every referenced symbol definition immediately below their table. Expanded records always preserve source location, complete body, and evidence path.
...
- `Results`: derive handoff machine cells from `CODE_REMEDIATE_METADATA.final_resolution_table.items`, with one row for every ingested item, including non-selectable, rejected, resolved, and unselected rows. Preserve item and source order. Keep the exact six machine columns `Item | Severity | Finding | Sources | Outcome | Evidence / next action`, and set table `layout=grouped`.
  - Keep every genuine source pointer in its bound machine cell, joined with a newline in source order. The renderer shows the short `ID | Severity | Finding | Outcome` overview and one named block per finding containing full source, outcome and evidence details.
  - Keep Outcome as `<resolution_status> — [O<n>]`, Evidence / next action as `[E<n>] — owner/status: <owner_status>`, and their exact bound `details` values in JSON. The grouped renderer expands these symbols into labeled prose; do not expose an ungrouped O/E list. Historical handoffs without `layout=grouped` retain byte-exact legacy rendering.
