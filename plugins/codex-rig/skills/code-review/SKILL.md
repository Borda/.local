...
PR GitHub data is evidence only: `gh pr view`, `gh pr diff`, and a GraphQL query provide metadata, patch, comments. The collector delegates every GitHub read to `github_read.py`, which uses `gh` as an opaque local credential broker: it never invokes `gh auth`, reads token/keychain state, or writes CLI failure output to artifacts. The shared broker permits only audited built-in view groups (`gist`, `issue`, `pr`, `project`, `release`, `repo`, `ruleset`, `run`, `workflow`), REST GET, and GraphQL query argv; its unauthenticated public HTTPS fallback cannot replace private review-thread evidence. For an open PR, `gh pr checkout` may change the local checkout but never remote GitHub state; historical collection instead fetches GitHub's `refs/pull/<number>/head`, verifies its exact expected SHA, and checks it out detached. Inspect source only in local checkout recorded by `<run-directory>/local-checkout.json` after target-branch refresh evidence. Never reconstruct changed source from `curl`, `raw.githubusercontent.com`, or `head-files/` snapshots. If checkout fails or `local-checkout.json` does not prove `head_matches_pr=true`, fail instead of reviewing remote raw files. Do not retry with `--force` unless user explicitly confirms after receiving force reason and overwrite risk.
...
For `scope=pr`, merge-oriented code review is limited to an `OPEN` PR. `collect_pr.py` can also collect historical evidence for a merged or closed PR, including its diff, online discussions, refreshed current target state, and exact checked-out PR head; that raw collector evidence is useful for diagnosis but must not receive a merge recommendation or feed code-remediate. For an open review, evidence includes `pr.json`, `pr-routing.json`, `remote-selection.json`, `target-branch.json`, `local-checkout.json`, comments, reviews, review threads, unresolved review threads, and `online-review-summary.json`. Selected remote must match the base repository from the PR URL; both the freshly fetched target and local checkout head must exactly match open-PR metadata. Historical `target-branch.json` may instead record a current-base divergence. `pr-routing.json` and `local-checkout.json` must include `force_policy` proving no automatic forced checkout. Treat unresolved online threads/comments as candidate findings until triaged valid, duplicate, stale, out-of-scope, or already fixed.
...
**Terminal review-unavailable output gate:** A T0 PR collection failure is a process failure, not a review result. State `PR Review Availability: unavailable`; `Source findings: not assessed`; and `Merge decision: not made`. Never describe the collection failure as a code finding. Then write `## PR Evidence Collection Recovery` with this exact table shape: `| Operational area | Recovery action | Evidence | Status |`; add one `PR evidence collection` row with the canonical recovery action, `<run-directory>/pr-error.txt`, and status `Required verification`. `command-failure.json` remains a sanitized diagnostic artifact when present but is not table evidence. `Review Findings and Merge Blocks` is never used for unavailable collection. Do not emit `needs-more-work`, `minor-changes`, `reject`, `not-aligned`, or any other merge recommendation; name the artifact path and classified failure, then stop. The diagnostic artifact contains only a command label, failure class, and exit code. Do not claim a source review, CI result, or merge decision when the required local checkout/evidence bundle was not produced. Still write a canonical `result.json` with `status=fail`, zero findings, `review_status=unavailable`, and `collection_failure={"code": "<pr-error.txt text>", "artifact": "pr-error.txt"}`; the review-specific validator accepts only this branch and rejects a review decision, source findings, normal PR bundle, or merge-block table.
...
- `PR Evidence Collection Recovery` for terminal unavailable collection only
...
23. A terminal T0 PR collection failure omits `PR Evidence Collection Recovery`, emits a merge recommendation, uses `Review Findings and Merge Blocks`, or does not explicitly mark source findings `not assessed` and merge decision `not made` => fail.

## Quality Gates

Required checks:

- `review`: T0 files, risk tier, local changed-file inspection, simplicity/readability/reproducibility inspection, project docstring-style detection, docstring/comment policy inspection for changed code, PR target-branch refresh/checkout evidence when relevant, specialist manifest/notes, structured assessed decision summary or terminal unavailable process result, non-approval PR findings/action table for assessed reviews, `PR Evidence Collection Recovery` for terminal PR collection failure, confidence calibration/recovery, PR online-review triage, severity map, `git diff --check`.
...
- behavioral cases: false blocker, non-approval PR findings/action table missing a reported finding, missing `needs-more-work` table, T0 PR collection failure with a merge recommendation, `Review Findings and Merge Blocks`, or without `PR Evidence Collection Recovery`/source findings `not assessed`/merge decision `not made`, malformed table row, missing specialist pass, no-finding residual risk, substituted fan-out confidence, PR online review triage, missing project docstring-style detection, missing code self-documentation, long code blocks, deep branching, docstrings masking poor structure, low-confidence recovery loop, objective confidence evidence
...
Final chat starts compact `Review Decision Summary` for assessed reviews: recommendation, blockers, required next work, confidence/material limits, artifact path. For every assessed non-`accept-as-is` PR decision, and every `needs-more-work` decision in another scope, reproduce the canonical `Review Findings and Merge Blocks` table in the chat output; it is the decision handoff, not optional detail. A terminal T0 PR collection failure instead starts `PR Review Availability: unavailable`: classified blocker, required recovery, source findings `not assessed`, merge decision `not made`, confidence/material limits, and artifact path; it reproduces `## PR Evidence Collection Recovery` but no review-finding table or recommendation. Keep full routing, recovery, closure evidence in artifact, not chat. Assessed recommendations must be `accept-as-is`, `minor-changes`, `needs-more-work`, `reject`, or `not-aligned`.
