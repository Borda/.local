...
When several independent role passes are justified, Codex Rig fixes their routes and narrow context packs first, dispatches the approved work in one wave when the runtime supports it, and joins every handoff before parent acceptance. It never starts a second wave for the same parent work item; new discoveries stay parent-serial or require a user-visible re-plan. Unsafe or unavailable parallelism uses the same gates serially; scheduling never expands fan-out, overlaps ownership, or bypasses approval.
