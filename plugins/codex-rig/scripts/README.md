...
`scripts/` holds every executable and library module the Codex Rig plugin needs to build, validate, install, and run itself. It splits into two groups: seven public/maintainer CLIs you run directly from a terminal, and nine underscore-prefixed internal helpers that those CLIs (and each other) import but that are never invoked on their own.
...
**When-to-use:** After adding, removing, or editing any packaged file (a `ROLE.md`, a skill, a script) — run `--update` to refresh the manifest, then `--check` in CI or pre-commit to confirm the manifest still matches the tree. This is also the hook `codex-rig-package-manifest` runs automatically on save.
...
**Usage:** This module has no `argparse` entry point and no `if __name__ == "__main__"` guard — it is not a CLI. It is imported directly, by `manage_role_agents.py`, by the `_agent_shim_*` helpers, and by the test suite, for its public functions (`load_generated_roster`, `render_role_shims`, `generate_role_shims`, `roster_identity_hash`) and constants (`ROLE_IDS`, `RUNTIME_KEYS`, `FRONTMATTER_KEYS`).
...
All six value flags are required. On success stdout starts with a protocol-1 ok envelope for the role, followed by the `--- codex-rig-role-card ---` separator and the verified card bytes; on failure it prints a JSON object with `status: "codex-rig-role-unavailable"` and a reason.
