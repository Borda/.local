...
Session default, review parent, implementation, verification, data, performance, research, curation, adversarial-challenge specialists use `gpt-5.6-terra` at `high`. Delegation coordination, documentation, CI/CD stewardship, web-evidence, OSS triage, static-analysis specialists use `gpt-5.6-luna` at `high`; final behavior-changing and executable acceptance decisions stay with the Terra parent/session. `gpt-5.6-sol` at `high` remains pinned only for `security-auditor` and `solution-architect`, selected solely when the user expressly requests Sol or names one of those agents. A selected Sol pass stays read-only, returns bounded evidence/artifacts, and hands the next action and final acceptance back to Terra. Luna activation = explicit user preference, retained separate from recorded strict route failure.
...
05. For a small closed choice, prefer an explicit condition/mapping over a registry. When a verified boundary under rule 18 requires a local import, prefer a conditional or lazy import; catch only the expected missing optional dependency, and let nested/transitive import failures surface. Use a registry when discovery or extension is an actual requirement.
...
07. Do not force DRY. A little visible duplication is cheaper than premature coupling. An abstraction with one caller is valid when it creates a genuinely deep boundary.
08. Avoid low-value helpers and wrappers. Penalize functions/classes that only remap arguments, forward one call, or serve one trivial consumer. Prefer direct code, a caller-local helper, or `functools.partial` for argument binding.
...
17. Abstractions must reduce cognitive load and the number of concepts a reader must follow. Extract only stable repeated behavior, genuinely shared infrastructure, or irrelevant construction mechanics; keep behavior-defining inputs and outcomes explicit. Cover the complete related behavior already present, place the abstraction in the narrowest shared scope, and prefer small visible duplication over aliases, wrappers, factories, or layers that add indirection without semantic value.
18. Keep Python imports at module scope by default. Use a local import only for a verified circular-import boundary, optional-dependency boundary, import-behavior test, or material startup or side-effect constraint; make the reason evident from surrounding code or document it when it is not obvious.

### Codex Rig Module Documentation

Every shipped non-test Python module starts with a maintainer-facing module docstring containing `Purpose:`, `Scope:`, `Usage:`, `Outputs:`, `Failure:`, and `Used by:`. Describe the module boundary, inputs/outputs or artifact paths, side effects or deliberate lack of them, real CLI/import entrypoint, important failure/exit behavior, and the workflow or callers that consume it. The docstring must be substantial enough to orient a maintainer without first reading the implementation; Codex Rig enforces a 700-character minimum. Keep function docstrings focused on local contracts; module docstrings explain the system role. Tests are exempt from the six-section format but still need a concise module description.
...
- Fixtures return ready-to-use concrete state or a cohesive tuple of related state. Do not return a callable factory unless fixture-managed lifecycle requires it; use an ordinary helper function for configurable construction. Keep fixture dependencies minimal, unpack only the values a test needs, and avoid aliases or forwarding helpers that add no meaning.
...
- Keep shell network access blocked by default. For every intentionally networked CLI, execute the complete owning command with runtime-approved external access from its first attempt; wrappers own approval for nested subprocesses and HTTPS. In Codex exec calls use `sandbox_permissions="require_escalated"` with a narrow justification; never enable persistent workspace network access, request a broad interpreter prefix, or assume a nested executable's approval covers its parent. This includes every `gh` and `kaggle` invocation, collector-owned `git fetch`/HTTPS, Codex Git marketplace add/upgrade and its owning sync wrapper, and paid `codex exec`; web/browser/MCP/connector tools use their own permission path. Marketplace/plugin listing and `codex plugin add` from an existing snapshot stay sandboxed. Missing external CLIs are user-owned prerequisites: explain the required installation and authentication, but never install them from a workflow.
- Before every intentional approval request, give one short plugin-owned brief: `Action and purpose`; `External capability`; `Credential behavior`; `Filesystem and worktree effects`; `Retry policy and safe denial outcome`. Denial aborts the active tool call and may end the assistant turn. Do not issue an equivalent approval request in the current turn or switch to a broader command; do not enable persistent network access or report completion. Ask the user to send a new message to resume under the documented command boundary.
- For all intentional approval requests, keep the runtime `justification` or reason separate from the pre-brief: it must be a short plain-English question about the requested outcome or material effect and must not repeat the command, argv, flags, paths, multiline content, or full approval brief. A justified reusable `prefix_rule` must be a short categorical safe prefix, never the entire command; omit `prefix_rule` for one-time or high-risk commands.
- GitHub data reads use `shared/github_read.py` only. It treats `gh` as an opaque local credential broker: never run `gh auth`, read token/keychain/account state, or retain GitHub CLI stdout/stderr on failure. It permits only audited built-in view groups (`gist`, `issue`, `pr`, `project`, `release`, `repo`, `ruleset`, `run`, `workflow`), `gh api` GET requests, GraphQL queries, PR diffs, and local `gh pr checkout`; it rejects unlisted `gh * view` commands, remote mutation, browser-opening `--web`, non-GET REST calls, file-backed API fields, and GraphQL mutations. Use its unauthenticated public `api.github.com` GET fallback only as a final public-data fallback; private-only evidence fails closed. GitHub Discussions use an explicit read-only GraphQL query because `gh` has no `discussion view` command. Codex Git marketplace add/upgrade is a separate explicit lifecycle operation, not a GitHub data read.
- Keep `collect_pr.py` as the only resource-specific GitHub collector unless another workflow demonstrably needs a composite, validated evidence bundle or local-state operation. Issues, releases, repositories, and Discussions use `github_read.py` directly. A new collector needs a written bundle contract, consumer workflow, and regression tests; do not create parity wrappers around a single read.
...
- After `investigate`, hand off to relevant domain agent or `implement`/`code-remediate` with evidence summary. Temporary mitigations only when explicit requested or required to unblock verification; never treated as root fix.
