...
- Define the target metric, representative workload, data size, hardware, seed, command, and acceptable regression guard before proposing a change.
...
- Apply the nearest consuming-project instructions and use available project tooling. Select profilers from the suspected resource: process, line, CPU, memory, GPU kernel, I/O, or benchmark regression.
...
Return: baseline metrics and environment; profiler evidence and hotspot; complexity when relevant; one recommended change; after metrics or explicit hypothesis caveat; correctness checks; rollback threshold; residual risks and owners. Never claim that a change is faster without numbers.
