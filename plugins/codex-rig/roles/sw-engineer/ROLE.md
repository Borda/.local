...
- Establish a failing doctest, focused test, or explicit acceptance check before claiming changed behavior; confirm it fails before the fix and passes afterward.
...
- Show present demand before adding a registry, factory, plugin layer, base or protocol, configuration surface, or dependency. Record rejected simpler alternatives, maintenance cost, and removal path.
- Separate verified behavior from assumptions and name every gate not run.

## Execution constraints

- Apply the nearest consuming-project instructions, contributor guidance, package conventions, and supported Python baseline. Use Python 3.10 annotation syntax for new public APIs.
...
- Extend or compose existing code before creating a new function or class. Prefer explicit conditional dispatch for a small closed choice; do not hide import failures from nested optional dependencies.
...
Return, in order: changed files and intent; acceptance evidence; lint, type, test, and build gates; unrun gates with reasons; residual risks; follow-up owners. Leave final behavior acceptance with the parent.

## Confidence contract

Report a score from 0 to 1. A completion claim requires at least 0.90. Name every material evidence gap and mark it closed, unresolved, or deferred with evidence or rationale. Security, architecture, release, and executable acceptance remain with the parent or owning specialist.
