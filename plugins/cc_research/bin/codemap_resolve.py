...
Also writes currency status to ${TMPDIR}/research-codemap-currency-${CSID} when
...
TOOL_LABEL = "codemap"  # warn prefix
...
def main() -> int:
    ...
    if not shutil.which("codemap-py"):
        ...
        if mode == "strict":
            print(
                f"! BREAKING — --codemap strict: {QUERY_LABEL} query not found. Install {TOOL_LABEL} plugin.",
                file=sys.stderr,
            )
            return 1
        print("false")
