...
Per-target checkpoint init — create immediately before spawning each pipeline (sequential execution: only one runs at a time, do NOT pre-initialize checkpoints for unstarted targets):
```bash
...
`! No prior run for <target> — re-run with --fast --apply to benchmark+apply, or --fast to benchmark only. (If target was skipped because its plugin was unavailable, install the plugin first, then re-run.)`
