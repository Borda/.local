...
def _locate_helper(home: Path, env_root: str | None) -> Path | None:
    ...
    3. Source-tree dev fallback ``plugins/foundry/bin/get_plugin_install_path.py``.
    ...
    source_fallback = Path("plugins/foundry/bin/get_plugin_install_path.py")
...
def resolve(plugin: str, subdir: str, *, home: Path | None = None, env_root: str | None = None) -> tuple[str, int]:
    ...
    if hit is not None:
        return str(hit), 2
    source_fallback = Path("plugins") / plugin / subdir
