...
2. ``plugins/foundry/skills/<skill>/<subdir>`` when ``--local`` is passed and
...
Options:
    --local  Prefer ``plugins/foundry/skills/<skill>/<subdir>`` after
...
def _from_local_source(skill: str, subdir: str) -> Path | None:
    """Resolve via the in-tree ``plugins/foundry`` source layout.
    ...
    Returns:
        The candidate path when it exists as a directory, else ``None``.
    """
    candidate = Path("plugins") / "foundry" / "skills" / skill / subdir
    return candidate if candidate.is_dir() else None
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--local",
        action="store_true",
        help="Prefer plugins/foundry/skills/<skill>/<subdir> (plugin-dev workflows).",
    )
    parser.add_argument(
        "--home",
        default=None,
