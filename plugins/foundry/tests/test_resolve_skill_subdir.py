...
def cwd_with_local_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Chdir to a fake project root containing a ``plugins/foundry/skills`` tree."""
...
class TestResolveCascade:
    ...
    def test_tier2_local_skipped_when_local_false(
        ...
        local = cwd_with_local_tree / "plugins" / "foundry" / "skills" / "audit" / "templates"
    ...
    def test_tier2_local_hit_when_local_true(
        ...
        assert result == Path("plugins") / "foundry" / "skills" / "audit" / "templates"
...
class TestMain:
    ...
    def test_local_flag_propagates(
        ...
        monkeypatch.delenv("CLAUDE_PLUGIN_ROOT", raising=False)
        (cwd_with_local_tree / "plugins" / "foundry" / "skills" / "manage" / "templates").mkdir(parents=True)

        rc = main(["manage", "templates", "--local", "--home", str(fake_home)])

        assert rc == 0
        out = capsys.readouterr().out.strip()
        assert out == "plugins/foundry/skills/manage/templates"
