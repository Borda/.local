...
DEFAULT_CACHE_GLOB = "borda-ai-rig/codemap-py/*"
...
def resolve_plugin_root(explicit: str | None) -> Path | None:
    ...
    ``~/.claude/plugins/cache/borda-ai-rig/codemap-py/*/claude-skills/integration``.
...
def detect_dual_identity(cache_base: Path | None = None) -> str | None:
    """Return the ``dual_plugin_identity`` violation when both plugin identities are cached.

    A cache holding BOTH the legacy ``codemap`` and the renamed ``codemap-py`` install
    dirs is the migration's dual-identity ban. The renamed glob is the shipped
    :data:`DEFAULT_CACHE_GLOB`; the legacy glob is DERIVED from it (never re-hardcoded),
    so both anchor on one source of truth.

    This is DETECTION only — a non-fatal named finding. Orchestration-level rejection
    (refusing operations while both exist) is Phase 4 ``integrate``.

    Args:
        cache_base: the ``plugins/cache`` directory to inspect; defaults to
            ``~/.claude/plugins/cache``.

    Returns:
        ``"dual_plugin_identity"`` when both identities are present, else ``None``.
    """
    if cache_base is None:
        home = Path(os.environ.get("HOME") or os.path.expanduser("~"))
        cache_base = home / ".claude" / "plugins" / "cache"
    legacy_glob = DEFAULT_CACHE_GLOB.replace("codemap-py", "codemap")
    if any(cache_base.glob(DEFAULT_CACHE_GLOB)) and any(cache_base.glob(legacy_glob)):
...
def audit_recorded_sites(sites: list[str], base_dir: Path) -> list[str]:
    ...
    for site in sites:
        ...
        if not _file_has_marker(path, SCAN_QUERY_MARKER):
            lines.append(f"  ⚠ MISSING injection: {site} — run /codemap-py:integration init to (re)inject")
...
def _skill_status_lines(skill_files: list[Path], cache_prefix: str) -> list[str]:
    ...
    if not skill_files:
        return [
            ...
            "  → Run /codemap-py:integration init to add injection",
        ]
    ...
    if outdated:
        ...
        lines.append("  → Run /codemap-py:integration init to re-inject the current block")
    return lines
...
def build_audit_lines(cache: Path, integration_dir: Path | None = None) -> list[str]:
    ...
    dual = detect_dual_identity(cache.parent)
    if dual:
        lines.append(
            f"  ⚠ {dual}: both legacy codemap and codemap-py plugin caches present — "
            "remove the legacy install (Phase 4 integrate will refuse operations while both exist)"
        )
    ...
    lines.extend(
        [
            ...
            "--- check complete ---",
            "If any check failed:",
            "  • /codemap-py:scan-codebase    — build or refresh the index",
            "  • /codemap-py:integration init — add injection to more skills/agents",
            "  • /codemap-py:integration check — re-run after fixes",
        ]
    )
    return lines
