...
**Codex integration study** — 55 shared tasks × 1 repetition × 3 treatments (`A_plain`, `B_direct`, `C_skill`) = 165 cells on 2026-08-02, using codemap-py 0.27.0, codex-rig 0.4.1, Codex CLI 0.146.0, and `gpt-5.6-luna` at high effort on `pytorch-lightning` 2.6.5. On 45 headline tasks, descriptive quality/input/elapsed means were A `38/45 · 0.955 · 169.8k · 81.8s`, B `36/45 · 0.944 · 144.2k · 55.6s`, and C `35/45 · 0.919 · 102.3k · 52.4s`; clean-triplet bootstrap intervals include zero for all quality differences. The run completed all cells but is not eligible for canonical pooling because it contains two required-use misses and four extraction failures, one repetition, cache/order exposure, and source-launch bytes differing from the current manifest/runner. Treat it as descriptive integration evidence only; it does not demonstrate post-fix quality parity or a confirmed Codemap advantage.

The failures were actionable and reproducible: Skill routing selected the wrong `central`/`fn-blast` query family; static alias/re-export completeness was lost; evaluator parsing and DiffImpact staging diverged from the shared contract; and `n=1` left order/cache variance unresolved. Recovery is implemented in the shared product surfaces: schema 12 records alias/re-export limitations, bounded compact output reports limitation counts with bounded samples, Claude/Codex Skill routing is mirrored with recovery guidance, evaluator and staging logic are shared, semantic query admission is enforced, and provenance/index validation fails closed. Completed no-model evidence is the exact 54-coordinate `bash benchmarks/run-all.sh codex --diagnostic --dry-run`, which passed without a model or authentication source. The benchmark suite passed 1,570 tests with 4 skipped; the plugin suite passed 1,204 with 11 skipped. The active lock is published in the benchmark manifest, avoiding a self-referential package digest in this shipped README. Paid post-fix quality remains unknown and requires fresh manifest review and authorization; these changes do not retroactively rescore the historical run or imply a Claude quality change.
...
The next evidence is a separate, explicitly non-poolable 54-cell diagnostic over `DI-01`, `DI-05`, `DI-06`, `GR-01`, `GR-03`, and `GR-04`, each repeated three times across `A_plain`, `B_direct`, and `C_skill`. It will run only after manifest review and no-model dry-run, with fresh paid authorization. Full tables, failure analysis, and artifact checksums are in [`benchmarks/README.md#codex-integration-study-a-b-c`](../../benchmarks/README.md#codex-integration-study-a-b-c); local ignored artifacts are under `benchmarks/results/codex-integration-20260802T135019Z/`.
