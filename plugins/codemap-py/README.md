...
The failures were actionable and reproducible: Skill routing selected the wrong `central`/`fn-blast` query family; static alias/re-export completeness was lost; evaluator parsing and DiffImpact staging diverged from the shared contract; and `n=1` left order/cache variance unresolved. Recovery is implemented in the shared product surfaces: schema 12 records alias/re-export limitations, bounded compact output reports limitation counts with bounded samples, evaluator and staging logic are shared, and provenance/index validation fails closed. The completed post-fix diagnostic contains 54/54 cells over six DI/GR tasks with three repetitions: A/B/C mean quality is equal at `0.8945`; B/A paired geometric ratios are `0.4465` input and `0.3335` elapsed; C/A ratios are `0.2849` input and `0.2932` elapsed. This is strong targeted efficiency evidence, not a confirmatory verdict. It also revealed that only 11/36 B/C cells matched the exact locked query shape even though all 36 used Codemap transport. Treatment adherence and continuous task-query fitness are now separate, and both provider Skills route production direct callers through `fn-rdeps … --exclude-tests`, transitive callers through `fn-blast`, and production centrality through `central --top N --exclude-tests`.
...
The next evidence is a separate, explicitly non-poolable 27-cell validation over `DI-01`, `GR-01`, and `GR-03`, each repeated three times across `A_plain`, `B_direct`, and `C_skill`. It validates the repaired direct, transitive, and query-fitness paths before a full 165-cell rerun. Full tables, failure analysis, and artifact checksums are in [`benchmarks/README.md#codex-integration-study-a-b-c`](../../benchmarks/README.md#codex-integration-study-a-b-c); local ignored artifacts remain under `benchmarks/results/`.
