...
**Codex integration study** — the completed selected validation used the shared `DI-01`, `GR-01`, and `GR-03` task objects, prompts, evaluators, target, and ground truth with three repetitions across `A_plain`, `B_direct`, and `C_skill` (27 cells) on `pytorch-lightning` 2.6.5. It used `gpt-5.6-luna` at high effort, Codex CLI 0.146.0, codex-rig 0.4.1, and installed codemap-py `0.27.0`. The run is complete and integrity-valid but explicitly non-poolable. Its immutable raw telemetry is `aebf677437c6b65d04681ea1d67b52030710df44e91b9cad4f9097e8591bed69`, canonical telemetry is `15ab38a5fbda643032ee21bdf833b9229fe2e47856caeeffa5e2673ca743e5d3`, and metadata is `d9d8032fa10795935ae53811729adf50eb5fa82d79ce723b7549249a465bc0c3`. Corrected offline rescore `offline-rescore-v1-aebf677437c6b65d-cec6f4d18d3f5a6a.json` has SHA-256 `8458e5dc58957453fd3ba94507d27c8f2b1a8e9cdb4d6f2b5b205bd74b400e34` and derived SHA-256 `e9cca34f35da0672716bf12a6dd0fdd22e1d3158509856203d16cb93a2a1d987`.

The selected run found 27/27 treatment-adherent cells after correcting the parser; seven semantic query-shape misses remain: C on `DI-01` (three repetitions), B on `DI-01` (one), and B on `GR-01` (three). Corrected quality means are `DI-01`: A `0.367`, B `0.500`, C `0.333` (no binary-correct repetition because the answer used the wrong test-module identity); `GR-01`: A `0.767`, B `0.700`, C `0.700` (3/3 binary-correct each); and `GR-03`: `1.000` for all treatments. These are targeted diagnostic results, not evidence of a general Codemap advantage or confirmatory parity. The reproducible root fixes are now in the shared product surfaces: the parser recognizes unquoted `$CODEMAP_BIN`, Markdown message boundaries are preserved, DiffImpact scores provider-neutral caller-and-test precision/recall F1, scanner identity normalizes `tests_fabric...`, `central --exclude-tests` recomputes production in-degree, direct guidance uses `fn-rdeps` while explicit transitive guidance uses `fn-blast`, and offline rescore fails closed on incomplete or mutable inputs.

Source-transfer proof is deterministic: the 74-file Codemap package passed package-manifest validation, and a disposable Codex install/runtime probe passed for the six Codemap skills against schema 12. The remediation candidate tested before this release was `0.27.0+codex.20260803165016`; its Codex query Skill matched source at SHA-256 `ca5112c5b1adc03ba063797fc00855d1acc418ef45fad8c6bd54385fcab7688c`. No user authentication source was read.

The subsequent, explicitly non-poolable 18-cell validation over `DI-01` and `GR-01` completed with all 491 checksums valid and no treatment, extraction, contamination, completeness, or token-accounting failures. DI quality A/B/C was `0.500/1.000/0.501`; GR was `0.733/0.600/1.000`. Paired C/A input ratios were `1.008×` on DI and `0.197×` on GR; elapsed ratios were `0.989×` and `0.089×`. The aggregate direction is promising, but the task-local split exposed two deterministic validity defects: GR's shared prompt did not state the oracle's test exclusion, and DI's Skill route used transitive module `test-impact` for a direct-import request. The semantic audit also covered only the caller half of DI. Both runtime Skills now route direct test importers through `rdeps <module>`, all DI tasks lock caller plus direct-importer queries under an all-required policy, and the GR prompt is provider-neutrally explicit. The corrected no-model gate passed; one new bounded validation is required before the full 165-cell study. Current lock and scope hashes live in [`benchmarks/README.md#codex-integration-study-a-b-c`](../../benchmarks/README.md#codex-integration-study-a-b-c) so this packaged README does not create a self-referential package hash; local ignored artifacts remain under `benchmarks/results/`.
