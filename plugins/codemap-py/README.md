...
The complete ReadCrop, localized-fix, and Patch suites are accepted as separate provider/model strata. ReadCrop preserves answer quality while generally reducing strict-arm context and command use. Localized fixes and Patch intentionally retain neutral and unfavorable efficiency cells: when an exact file and symbol leave no unresolved caller, dependency, import, test-impact, or source-slice fact, the installed guidance skips Codemap instead of paying for forced retrieval. Revised multi-file evidence is heterogeneous: Claude Haiku and Sonnet show strong strict-arm reductions on the valid FM-03 affected-surface pair, Sonnet strict fails FM-01 where plain passes, and Codex strict preserves correctness but does not reduce aggregate input. In the current Patch recalibration, Claude's two valid A/C pairs reduce strict context/time with two additional strict-only successes, while Codex's three quality-tied A/C pairs reduce gross input, output, commands, and elapsed time but retain two strict oracle failures. The benchmark therefore supports adaptive structural retrieval rather than a universal editing or token-efficiency advantage.
