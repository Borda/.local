...
def _corpus_files() -> list[Path]:
    ...
    return sorted(p for p in _CORPUS.glob("*.py") if p.name != "conftest.py")
...
def _make_corpus_tree(tmp_path: Path) -> Path:
...
def test_corpus_is_present_and_named() -> None:
...
def test_corpus_contains_grammar_gated_post311_syntax() -> None:
...
def test_degraded_records_are_stable(tmp_path: Path) -> None:
...
def test_degradation_tracks_running_grammar(tmp_path: Path) -> None:
    ...
    """
    index = _scan(_make_corpus_tree(tmp_path), tmp_path / "idx")
...
def test_post311_modules_degrade_below_312(tmp_path: Path) -> None:
