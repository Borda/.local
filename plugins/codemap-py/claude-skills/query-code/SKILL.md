...
description: |
  ...
  SKIP: rename intent (use /codemap-py:rename-refs); simple grep suffices; non-Python repo. Missing/stale index auto-builds — no manual scan-codebase needed.
...
NOT for: repo rebuild (use `/codemap-py:scan-codebase`); renaming symbols (use `/codemap-py:rename-refs`); non-Python files.
</objective>
...
if [ "${SCAN_NO_AUTOBUILD:-0}" = "1" ]; then
    ...
    _CM_BUILD_T0=$(date +%s)
    "${CLAUDE_PLUGIN_ROOT:-plugins/codemap-py}/bin/scan-index" --incremental
...
if [ "${SCAN_NO_AUTOBUILD:-0}" = "1" ]; then
    ...
    echo "  Build it manually first: /codemap-py:scan-codebase"
    exit 1
fi
```

If not refused → run `scan-index` in the foreground (wait until it finishes), then re-read INDEX from tmpfile. (Not the `codemap-py:scan-codebase` skill — it is `disable-model-invocation:true`, user-slash-only; build via the `scan-index` binary.)
...
Missing/old index → query needing newer version exits with "requires vN+ index" message; re-run auto-build (Step 0) or `/codemap-py:scan-codebase` to upgrade.
...
Symbol staleness: `stale: true` + empty source → `Read(path)` fallback. `stale: false` + empty → `[source not available — re-run /codemap-py:scan-codebase]`.
...
`index.stale: true` → scan-query already attempted bounded inline self-heal (`scan-index --incremental`) before answering; `stale` remaining true means heal was skipped (change set over cap, or git unavailable). Re-run `/codemap-py:scan-codebase --incremental` manually, then retry.
...
Next steps: /codemap-py:query-code <follow-up> or /codemap-py:rename-refs if renaming
