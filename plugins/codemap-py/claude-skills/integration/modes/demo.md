...
Validates codemap plugged in correctly, yields expected gains. State var `DEMO_MODE` (real | synthetic) drives D5a/D5b path selection.
...
Leave `CODEMAP_LOGGING=true` (default). Each bash block needing `$TARGET` must use separate `cd "$TARGET"` call first — never chain `cd "$TARGET" && command` (violates CLAUDE.md compound-bash rule; permission matcher only checks first token).
...
- `find . -maxdepth 3 -name "*.py" | head -1` finds file → `TARGET=$(git rev-parse --show-toplevel)`. Skip to D2.
- Otherwise → go to D1a.
...
Ask: "No local Python repo detected. Which public repo to clone for demo?" Options:
- (a) Cancel — stop demo
...
Run bin scripts inline — demo controls cwd, parses C3 JSON directly. First resolve index path so smoke script receives `--index-path`.

```bash
# timeout: 5000
export CSID="${CLAUDE_CODE_SESSION_ID:-$PPID}"
_DEMO_PROJ=$(git -C "$TARGET" rev-parse --show-toplevel 2>/dev/null | xargs basename 2>/dev/null || basename "$TARGET")
SQ=$(python "${CLAUDE_PLUGIN_ROOT:-plugins/codemap-py}/bin/locate_scan_query.py")
...
```bash
# timeout: 15000
C3_JSON=$(python "${CLAUDE_PLUGIN_ROOT:-plugins/codemap-py}/bin/check_index_smoke.py" \
    --index-path "${_DEMO_INDEX}" 2>&1)
```

```bash
# timeout: 10000
C4_OUT=$(python "${CLAUDE_PLUGIN_ROOT:-plugins/codemap-py}/bin/check_injection.py" 2>&1)
```
...
Parse `WIRED_FILES[]` from C4 stdout — every line matching `.md` path with injection marker:
...
`scan_query_ok=false` → print remediation hint (`claude plugin install codemap@borda-ai-rig`) and stop.
...
**`false`**: invoke `AskUserQuestion`: "Codemap not wired into any installed skills yet. Demo can run `init` now to wire it in, or proceed with synthetic A/B only." Options:
...
On (a): `Skill(skill="codemap-py:integration", args="init")`. After init, re-run C4:

```bash
cd "$TARGET"
C4_OUT=$(python "${CLAUDE_PLUGIN_ROOT:-plugins/codemap-py}/bin/check_injection.py" 2>&1)  # timeout: 10000
```

Re-derive `injection_present` and `WIRED_FILES[]`. `injection_present=true` → `DEMO_MODE=real`. Still false → `DEMO_MODE=synthetic`; record "init ran but no files were wired" as finding.
...
```bash
# scan-index binary — codemap-py:scan-codebase skill is disable-model-invocation:true (user-slash-only)
...
Load `${CLAUDE_PLUGIN_ROOT:-plugins/codemap-py}/claude-skills/integration/templates/demo-tasks.json`. Select task set by `basename "$TARGET"`:
...
```bash
Skill(skill="codemap-py:query-code", args="central --top 3")
```

PreToolUse hook writes one `skills.jsonl` entry, stamps `${TMPDIR}/codemap-<proj>-session`.

## D5a — Real-skill probe

**Run only when `DEMO_MODE=real` and `CLONED=false`.** Goal: prove codemap injection fires inside user's actual installed skill — not a synthetic agent.
...
Selection precedence (first that resolves wins). Set `PROBE_SOURCE` alongside `PICKED_SKILL` — drives D5a confidence tier and D8 "which probe ran" line:

1. **`--probe-skill <name>` (user arg)** — `PROBE_SKILL` set AND appears in resolved `WIRED_FILES[]` names → `PICKED_SKILL=$PROBE_SKILL`, `PROBE_SOURCE=user-arg`. Set but not wired: print `⚠ --probe-skill <name> not wired; falling back to priority list`, continue to step 2.
...
Confidence tier by `PROBE_SOURCE` (feeds D8 report-header Confidence): `user-arg` (operator pinned real installed skill, strongest signal) > `priority-list` (real skill, auto-picked) > `synthetic` (no real-skill probe ran). State chosen probe and its source verbatim in D8 Real-skill probe section.
...
For `develop:review`: use `git diff HEAD~1 --name-only | grep '\.py$' | head -1`. Empty → skip to next priority skill. For `oss:review`: requires PR number — skip (not suitable for demo).
...
Parse new cli.jsonl lines for subcommand breakdown and timing.
...
Arm prompts (plain / codemap), recall scoring formula, honest caveat printed verbatim in D8, and benchmark-upgrade offer: demo-notes.md § D5b. Run both arms per `ab_tasks` entry from `templates/demo-tasks.json`.

### Correctness signal

Signal type drives D8 report-header Confidence cap — must be reported honestly, never inflated:
...
Each arm runs under own session, so its scan-query / Grep / Read / Glob calls land in that session's telemetry shards. Assign distinct session id per arm before running it, so tool-use and cli hooks stamp arm's records:
...
# timeout: 15000
cd "$TARGET"
AB_MEASURED=$(python "${CLAUDE_PLUGIN_ROOT:-plugins/codemap-py}/bin/measure_demo_arms.py" \
...
`AB_MEASURED` JSON carries `plain.tokens`, `codemap.tokens`, `token_delta`, effective `signal_type` (downgraded to `plumbing` if either arm produced no telemetry), and capped `confidence`. Feed `plain.tokens` / `codemap.tokens` into D8 A/B table and `signal_type` + `confidence` into D8 report-header Confidence line.
...
_TODAY=$(date +%Y-%m-%d)
Skill(skill="codemap-py:debrief-coding", args="--since ${_TODAY}${ANONYMIZE_FLAG}")
```
...
**Report-header Confidence**: measured A/B signal sets ceiling (`confidence` field of `AB_MEASURED`): `ground_truth` → up to 0.9, `agreement` → cap 0.7, `plumbing` (either arm produced no telemetry) → cap 0.5. D5a probe source may only *lower* it — `synthetic` probe source (no real-skill probe ran) subtracts 0.1, `priority-list` source neutral, `user-arg` source (operator pinned wired skill) keeps ceiling. Never report above `AB_MEASURED` cap. State driving signal type and probe source in header `Confidence` gaps note.

Write report with Write tool using full report skeleton (YAML header + Plumbing / Index / Sample tasks / Real-skill probe / Synthetic A/B / Telemetry health / Debrief sections): demo-notes.md § D8 report template. Fill placeholders from values collected in D2–D7; omit Real-skill probe section when `DEMO_MODE=synthetic`. In Synthetic A/B table use measured `plain.tokens` / `codemap.tokens` from `AB_MEASURED` (not any self-reported arm count), print `signal_type` label next to correctness column. Print report path on completion.
