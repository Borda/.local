...
Set `TARGET` from `--repo` (path case) or after D1a clone. Resolve `SQ` via `bin/locate_scan_query.py`. Init `CLONED=false`, `DEMO_MODE=synthetic`.
...
Priority: `develop:fix` > `develop:refactor` > `develop:review` > `oss:review` > `develop:feature`. Pick first match in `WIRED_FILES[]` resolved names. No match: log warning, skip D5a, run D5b only.
...
Arm prompts (plain / codemap), recall scoring formula, honest caveat printed verbatim in D8, and the benchmark-upgrade offer: demo-notes.md § D5b. Run both arms per `ab_tasks` entry from `templates/demo-tasks.json`; score recall vs `task.ground_truth` where available, else `cross_arm_agreement`.
...
Write report with Write tool using the full report skeleton (YAML header + Plumbing / Index / Sample tasks / Real-skill probe / Synthetic A/B / Telemetry health / Debrief sections): demo-notes.md § D8 report template. Fill placeholders from the values collected in D2–D7; omit the Real-skill probe section when `DEMO_MODE=synthetic`. Print report path on completion.
