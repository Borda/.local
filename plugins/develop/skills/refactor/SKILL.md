...
Increment `TOTAL_CYCLES` each time a Step 4 change-test cycle completes. Check against `MAX_AGGREGATE_CYCLES` before each new cycle — stop if `TOTAL_CYCLES -ge $MAX_AGGREGATE_CYCLES`.
