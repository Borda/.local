...
Lightweight axis scorer for oss:analyse vitality. Reads pre-fetched raw JSONL and scores the assigned axis group per vitality-scoring.md rubric. Writes partial scores JSON. Runs in parallel with 2 other repo-warden instances.

NOT for data fetching — all raw data comes from DATA_FILE written by oss:gh-scraper.
...
- `GH_OWNER=<owner>` — GitHub owner or org (for log messages only)
- `GH_REPO=<repo>` — GitHub repository name (for log messages only)
- `DATA_FILE=<path>` — path to JSONL written by oss:gh-scraper
- `PARTIAL_FILE=<path>` — output path for this group's partial scores JSON
- `AXIS_GROUP=A|B|C` — which axes to score
...
Read `$DATA_FILE` fully using the Read tool. Parse JSONL records into in-memory structures needed for the assigned axis group.

For **Group A** (Axes 1, 2, 5, 6): extract records with type `responsiveness_gql`, `commits`, `releases`, `ci_workflows`, `ci_runs`, `repo_metadata`. Root file list comes from `repo_metadata` or a separate `contents` record if present. README and workflow content come from type `readme_content` and `workflow_files` if written by gh-scraper; otherwise infer from `ci_workflows` names.

For **Group B** (Axes 4, 7, 8): extract records with type `open_issues`, `closed_issues`, `open_prs`, `closed_prs`, `review_coverage_gql`, `dependabot_alerts`, `repo_metadata`. Root file list from `repo_metadata`. Governance files from type `root_contents`, `github_dir`, `codeowners_content`, `branch_protection`, `dependabot_config` if present.

For **Group C** (Axes 3, 9): extract records with type `contributor_stats`, `merged_prs_90d`, `commits_50`, `releases`, `fork_dates`, `star_dates`, `open_issues` (reused for 9C).
...
Read `$_OSS_SHARED/vitality-scoring.md` fully. Score each axis in the assigned group per its rubric. Use the raw data parsed in Step 2 as input.

**Group A** — score in any order (all independent within group; no cross-axis dependency; single agent, no internal parallelism needed):
1. Axis 1 — Responsiveness: use `responsiveness_gql` data; compute median_issue_response_days, median_pr_response_days, pct_responded_7d, pct_unresponded per rubric; exclude author's own responses
2. Axis 2 — Maintenance Activity: use `commits` dates and `releases` data; compute days_since_last_commit, commits_30d, commits_90d, release cadence
...
**Group B** — score in any order (all independent within group; no cross-axis dependency):
...
1. Axis 3 — Contributor Health: use `contributor_stats` (weeks[] data); filter bots; compute bus_factor, top_contributor_pct, retention_rate; apply 202-fallback from `commits_50` if stats unavailable; after scoring, write weeks[] array to `PARTIAL_FILE` as `axis3_weeks` field (bash variables do not persist across tool calls — always persist via file)
2. Axis 9 — Trajectory: after Axis 3 complete, score all 4 sub-signals:
   - 9A (reviewer pool drift): reads `axis3_weeks` from `PARTIAL_FILE` written by Axis 3 above (not from a bash variable); compute shrinkage_ratio from pool_recent vs pool_prior; if Axis 3 used fallback (`axis3_weeks: null`), mark 9A ⚪
...
For each axis, produce a result object:
...
Unavailable axes (all API calls failed for that axis):
...
For ⚪ axes, set `score: null` and `conf: 0.0` in the partial file (assembler treats null as excluded from health score).
...
Write `$PARTIAL_FILE` using the Write tool. Format:
...
Group C sets `axis3_weeks` to the actual weeks[] array from contributor stats (or `null` when fallback was used). The assembler reads this field for confidence display.
...
Compute group confidence as mean of per-axis confidence values for axes scored (exclude ⚪ axes with conf=0.0 from mean, or if all ⚪ return 0.0). Cap: when fewer than half the assigned axes are scored (e.g. 1 of 4 in Group A), cap group confidence at 0.7 to reflect incomplete coverage.
...
- **Group C sequential rule**: Axis 3 MUST be scored before Axis 9 — sub-signal 9A needs Axis 3 weeks[] data; if Axis 3 used commit-author fallback, mark 9A ⚪
- **Groups A and B fully independent** — order within group is notational only; no cross-dependency
...
- **Bot filtering**: applies in Axes 3, 4, 7 (checkpoint 7), 9A, 9B, 9D — exclude logins matching `*[bot]` or `*-bot` suffix; use bash pattern matching (`[[ "$login" == *"[bot]"* ]] || [[ "$login" == *"-bot" ]]`) — no jq or python3 required for the filter itself
...
- **Axis 3 fallback**: stats 202 after all retries → use commit-author approximation from `commits_50`; bus_factor approximation = count of distinct commit authors in commits_50 contributing ≥5% of total commits; mark conf=0.5; always attempt fallback before marking ⚪
- **Axis 8 partial scoring**: Dependabot 403 → partial_score formula from rubric; conf=0.4; never mark ⚪ solely from Dependabot 403
- **axis3_weeks field**: Group C must populate this even if Axis 9 uses it; set to `null` when fallback was used (no weeks[] available); PARTIAL_FILE paths are assigned by the spawning skill (oss:analyse vitality) with distinct suffixes per group (e.g., -group-A.json, -group-B.json, -group-C.json) — concurrent writes do not collide
