...
- **Axis 9 must be 🔴 for all abandoned repos** — pool_recent = 0, TTM no merges last 30d, and P90 queue age > 180d together guarantee 🔴; any 🟢/🟡 = scoring bug
- **Axis 9 sub-signal independence**: a 🔴 on sub-signal 9B (no merges 30d) alone pulls axis score below 7.5 even when other sub-signals are 🟢 — verify this composite behavior
...
- **Axis 9 stats dependency**: sub-signal 9A (reviewer pool drift) depends on Axis 3 contributor stats weeks[]; if stats 202 persists after retries, 9A is unavailable — axis scores on remaining 3 sub-signals with confidence -0.2; report must note "sub-signal 9A unavailable — contributor stats computing"
- **Squash-merge repos**: commit substance ratio (9D) may undercount dep-bumps when PRs are squash-merged under generic titles — known limitation; note if detected (large PR count, low distinct commit count relative to PR count)
...
- **High-volume dep-bump repos**: 9D 🔴 is correct signal that human commit output is maintenance-only; combined with 9A stable pool may indicate healthy automated maintenance — note both signals in Key Signal column
