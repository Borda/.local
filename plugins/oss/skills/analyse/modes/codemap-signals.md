...
python "${CLAUDE_PLUGIN_ROOT:-plugins/oss}/bin/detect_codemap.py" --prefix analyse 2>&1  # timeout: 5000
...
if [ "$CM_ENABLED" = "true" ] && [ -s "$_CAND" ]; then
    _BATCH_LEN=$(python "${CLAUDE_PLUGIN_ROOT:-plugins/oss}/bin/build_triage_batch.py" "$_CAND" "$_BATCH" 2>/dev/null || echo 0)  # timeout: 5000
