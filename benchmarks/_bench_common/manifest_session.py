...
Several collected modules read ``benchmarks/manifests/*.json`` at import time, so the
build has to happen before collection rather than in a fixture. ``benchmarks/conftest.py``
owns the hooks; this module owns the work so it stays importable by its own tests.
"""
