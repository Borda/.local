...
The current source relock is methodology SHA-256 `920f82eb626febf6b19bd2178b064f892be4b26f5d61bbc2d200f70b4d87b211`, machine-manifest SHA-256 `8388c82caa2b773e748441b194893b6691102e4fbcfe78b39f4b1f855f040d03`, and human-manifest SHA-256 `4ccbc3ab28a318766e77385ad7ed8bc67b27987f96475d78f69459bcc613da18`. The replacement no-model `DI-01,GR-01` gate passes with 18 planned cells, a 10,800-second ceiling, scope SHA-256 `406820ad0216ed7aacaaa0e7ed15159a725ce3498a3ebccd05952dca58641b9f`, and dry-run output SHA-256 `abd3d0de9aa0e1e6670f46937d4c8127e42c5cfd03edeeb25113c5e2e1f80f8e`. The installed Codemap candidate is `0.27.0+codex.20260803165016`; its Codex Skill matches the source at SHA-256 `ca5112c5b1adc03ba063797fc00855d1acc418ef45fad8c6bd54385fcab7688c`. The preceding paid 18-cell validation completed cleanly but exposed a provider-neutral GR prompt mismatch, overly broad DI Skill routing, and a compound-query audit blind spot; the full 165-cell study remains blocked pending one bounded validation of this corrected contract.
...
Final no-model verification is green: the complete benchmark suite passed (1,621 passed, 4 skipped) and the complete Codemap suite passed (1,214 passed, 11 skipped). Ruff, targeted Python compilation, Bash syntax, diff, package build/validation, installed-cache identity, and generated-manifest checks pass. Generic plugin-creator validation remains a non-blocking residual because it does not model the existing multi-runtime `codex-skills/` layout; no layout change is proposed.
...
The completed selected Codex run used the same frozen task objects, prompts, evaluators, target tree, and ground truth as the Claude structural benchmark. It is a descriptive integration result, not a pooled or confirmatory treatment claim.
...
| Software           | Paid run: codemap-py `0.27.0`, codex-rig 0.4.1, Codex CLI 0.146.0. Post-run remediation candidate: codemap-py `0.27.0+codex.20260803111514`.                                                                                                                                                                                         |
...
Corrected selected results (continuous quality is the mean per-cell fitness; input and elapsed values are arithmetic means):
...
**Validity and interpretation.** All 27 selected cells completed and passed artifact integrity, but the run is explicitly non-poolable. Corrected treatment adherence is 27/27. Seven semantic query-shape misses remain: C `DI-01` (3), B `DI-01` (1), and B `GR-01` (3). `DI-01` binary correctness is 0/3 for each arm because the returned test-module identity did not match the locked oracle; `GR-01` is 3/3 for each arm and `GR-03` is 3/3 for each arm. These targeted results do not establish a causal Codemap advantage, confirmatory effect, or cross-provider raw-token comparison. The offline rescore is derived evidence; the raw and canonical telemetry remain immutable.
