...
python3 benchmarks/run-claude-agentic.py \
    ...
    --index /private/tmp/codemap-provider-parity-pl-2.6.5/.cache/codemap/codemap-provider-parity-pl-2.6.5.json \
    --model haiku \
    --tasks FM-01,FM-03 \
    --dry-run
```
...
| Codex only       | `run-codex-structural.py`                    | One task-driven 68-task CLI: structural A/B/C plus separate ReadCrop, Fix-Single, and Fix-Multi stages |
...
| Provider parity                                                | Codex            | `run-codex-structural.py`  | Yes | Parity 3 (A/B/C)                       | 68 task IDs: 55 structural + 6 ReadCrop + 4 Fix-Single + 3 Fix-Multi; stages reported separately | Does Codemap provide an objective within-Codex advantage under the same shared contracts?             |
...
- `codex --struct` — validate the frozen index, run the fail-fast FN-02 A/B/C smoke and exact no-model plan, then execute all 68 tasks × one repetition × three arms. Structural, ReadCrop, Fix-Single, and Fix-Multi outcomes use native scorers and separate child artifacts; cell outcomes are recorded without fail-fast after admission. Paid execution requires the aggregate scope approval and private auth source; the launcher creates a fresh run directory, with an optional `CODEX_RUN_DIR` override for a new path. Each cell uses the retry-inclusive per-cell timeout in seconds; no total-run ceiling is configured.
...
`run-codex-structural.py` is task-driven: stage and execution mode are inferred from task selection and dry-run state. Omit `--tasks` to plan or run all 68 supported tasks (55 structural + 6 ReadCrop + 4 Fix-Single + 3 Fix-Multi), for 204 A/B/C cells at the default one repetition. Use family tokens for a stage or exact IDs for a targeted scope; mixed selectors are allowed, duplicates are removed, and locked manifest order is retained:

```bash
# All supported tasks: 68 tasks × 3 arms = 204 cells
python3 benchmarks/run-codex-structural.py --repo-path /path/to/pytorch-lightning --model gpt-5.6-luna --dry-run

# Selected families: ReadCrop + Fix-Single + Fix-Multi
python3 benchmarks/run-codex-structural.py --repo-path /path/to/pytorch-lightning --model gpt-5.6-luna --tasks RC,FS,FM --dry-run
...
Run the no-model `--dry-run` first; omit it only for the approved execution command. ReadCrop, Fix-Single, and Fix-Multi retain stage-specific scorers and separate, nonpoolable result artifacts. Empty selectors, unknown IDs/families, and invalid mixtures fail before target setup or model admission. `CODEX_PAID_APPROVAL` remains the manifest-bound authorization and stale-manifest lock; no separate boolean paid flag is needed.
...
Three suite files extend the benchmark from pure structural discovery into read-crop selection and the **edit phase** of real developer work:
...
Omitting `--tasks` runs all 68 locked tasks (204 A/B/C cells); use an explicit family, exact-ID, or mixed selector for a targeted nonpoolable scope. ReadCrop, Fix-Single, and Fix-Multi results retain separate stage artifacts and scorers.
