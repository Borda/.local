...
- Native target-project test pass rate after fixes; Codex executable stages use benchmark-owned AST behavior oracles because project dependencies are intentionally absent from the agent sandbox
...
| ----------------- | ----------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| `fix_single`      | `tasks-fix-single.json` | FS-01–FS-04 | Codex: clean patch apply + exact path + behavior oracle; Claude legacy: diff keyword recall (`erec`)                      | Single-file bug fix with independent executable validation        |
| `fix_multicaller` | `tasks-fix-multi.json`  | FM-01–FM-03 | Codex: complete-caller AST oracle + clean patch apply; Claude legacy: diff keyword recall (`erec`) + file recall (`rrec`) | Signature change + callers — codemap's edit-assist differentiator |

**Isolation**: both suites use `requires_reset: true`. The legacy Claude path copies the demo codebase and captures `diff -ru`; the Codex executable path creates a benchmark-owned disposable worktree, exposes only its checkout to the agent, captures the canonical Git diff outside the agent sandbox, applies it to a second clean scoring worktree, and verifies cleanup. The original codebase is never mutated.

**FM-03 is the decisive cross-file test**: `Strategy.setup` in pytorch-lightning has 6 production subclass overrides across `ddp.py`, `fsdp.py`, `deepspeed.py`, `model_parallel.py`, `single_xla.py`, and `xla.py`. The strict Codemap arm uses an unlimited same-name symbol query to discover candidates, then verifies inheritance and the production package boundary from source; `fn-rdeps` is not an inheritance query. The plain arm must discover the same surface with ordinary repository tools. Exact changed-path validation captures whether the right files were actually changed. This is the first benchmark family that directly measures whether Codemap reduces missed override candidates in a real multi-file edit.
...
**Scoring**: the legacy agentic path uses `score_fix()` to extract `+`-prefixed lines from `diff -ru`, check keyword hits (`erec`), and check file recall (`rrec`). Codex executable stages use the structural runner's isolated Git worktree executor instead: the baseline must fail its task-specific independent oracle; the patch must apply with ordinary `git apply --check`, touch exactly the allowed source paths in any order, pass that oracle, and leave no worktree behind. Fix-multi requires every caller or override declared by its contract. Task prompts preserve discovery as measured work rather than disclosing caller names or counts, and C receives a task-fit compact query: `fn-rdeps` for direct callers and unlimited `find-symbol` candidates plus source verification for overrides. These rows are separate stages, never pooled with structural answer-quality rows.
...
| Fix-Multi  | `2/3` / `3/3` |        `0.801×` |        `1.013×` |   `0.834×` |     `0.870×` |    `0.870×` | C recovers the hard affected-surface task and lowers aggregate cost.                                   |

Fix-Multi totals are scale-sensitive: the equal-task paired geometric C/A ratios are `0.980×` gross input, `1.096×` fresh input, `1.069×` output, `0.920×` commands, and `1.060×` elapsed. The aggregate saving comes from FM-03, where A produced an ordinary-applicable seven-file patch but failed the independent oracle; B and C passed, and C used `0.517×` gross input, `0.596×` output, `0.583×` commands, and `0.593×` elapsed relative to A. This supports adaptive routing rather than universal use: skip Codemap when a localized edit has no unresolved structural fact; query the smallest relevant caller, dependency, importer, source-slice, test-impact, or override-candidate surface when scope is uncertain. `C_strict` remains a forced-use treatment and therefore intentionally measures the localized-edit overhead rather than simulating that production skip decision.

`B_auto` remains a canary rather than the decision-grade comparison. In ReadCrop it used Codemap on all six tasks but cost `1.736×` A gross input and missed RC-03; in Fix-Single it made no Codemap calls and reproduced plain-like quality/cost; in Fix-Multi it used Codemap only on the hard task, passed 3/3, and cost `1.140×` A gross input overall. This pattern supports installed adaptive guidance over mere optional CLI availability.
