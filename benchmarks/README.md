...
The current source relock is methodology SHA-256 `2291f698f5dcf8cab34013068baa74a178a79cf78612ed7464188c255eabd193`, machine-manifest SHA-256 `22dd692577018e5d0f6848a516a8bad4fd2ed9b3c0a4dbb6594da00aa623b50b`, and human-manifest SHA-256 `541e36b29405f4706e4755aab0a5bc6fdc9b0b03f594c97b28cc05a9f0d8d78b`. The exact no-model selection passed for `bash benchmarks/run-all.sh codex --tasks=DI-01,GR-01,GR-03 --dry-run`: three tasks × three repetitions × A/B/C = 27 coordinates, a 16,200-second ceiling, and selection scope SHA-256 `9ca9ec5b0351d0c8bf57200dadd5e7ad689c3dd9ce69acc3c55145cf8ffbfac6`. These hashes bind the next user-launched validation; they do not describe a paid result.

Two paid post-fix diagnostic attempts stopped before any model cell: the first on macOS `/var` alias handling during snapshot creation, the second because the intentional `DI-01` stage conflicted with global clean-worktree admission. Both are infrastructure diagnostics, not treatment evidence. The repaired `DiffImpactStageAdmission` records exact Git status, repository commit, and intended-file SHA-256, restores the target in `finally`, and fails closed on unapproved changes or index/commit drift. The subsequent diagnostic completed 54/54 cells across `DI-01`, `DI-05`, `DI-06`, `GR-01`, `GR-03`, and `GR-04`: mean quality was equal at `0.8945` for A/B/C; paired geometric ratios were B/A `0.4465` input, `0.2732` output, `0.3335` elapsed and C/A `0.2849` input, `0.2623` output, `0.2932` elapsed. All 36 B/C cells used Codemap transport, but only 11/36 matched the exact locked query shape. This exposed a measurement bug: exact query-shape mismatch had been incorrectly folded into treatment adherence. The runner now preserves transport adherence and reports continuous `task_query_fitness` separately; exact semantic mismatches remain diagnostic counts and do not alone exclude a cell from pooling. Direct and Skill guidance now route production direct callers through `fn-rdeps … --exclude-tests`, transitive callers through `fn-blast`, and production centrality through `central --top N --exclude-tests`.
...
Focused verification is green: 351 runner/launcher/Skill-parity tests passed with 2 skipped; Ruff, compile, Bash syntax, diff, and generated-manifest checks pass. The complete benchmark and Codemap suites are rerun before each committed relock. Generic plugin-creator validation remains a non-blocking residual because it does not model the existing multi-runtime `codex-skills/` layout; no layout change is proposed.
...
Each cell receives an isolated `CODEX_HOME`. Permission profiles deny the copied credential, host agent roots, network, and source-tree writes; treatment arms may write only to the index-local coordination directory. The runner records answer, quality, extraction, treatment adherence, Codemap use, task-query fitness, and transport failures per cell and continues after the admission smoke so these outcomes remain measurable. Terminal output uses `treatment:✓|✗` for the assigned transport contract and `codemap-used:✓|✗` for observed Codemap use: clean `A_plain` is `treatment:✓ codemap-used:✗`; a contaminated A row can be `treatment:✗ codemap-used:✓`; B/C require the assigned direct or Skill delivery plus a successful compact query. Exact locked query-shape agreement is independently recorded as `codemap_semantic_compliance` and continuous Jaccard `task_query_fitness`, so a useful but non-exact query no longer masquerades as failure to receive the treatment.
...
The run exposed deterministic integration defects rather than one single model failure: Skill routing sent `central`/`fn-blast` work toward the wrong query family; the static graph did not preserve alias/re-export completeness; evaluator parsing and DiffImpact staging diverged from the shared contract; and one repetition left substantial order/cache variance. The source remediation records alias/re-export limits, bounds compact limitation samples, aligns evaluator and staging behavior, and fails closed on provenance/index drift. The completed 54-cell post-fix diagnostic then showed quality parity (`0.8945` in every arm) and large descriptive token/time reductions, while exposing that 25/36 B/C queries did not exactly match the locked task query. Because transport use was observed in all 36 B/C cells, the active runner separates assigned-treatment adherence from continuous task-query fitness instead of treating semantic mismatch as transport failure. It also executes paid work from a manifest-locked private launcher snapshot so an edited worktree script cannot change a running shell program. The active Codex machine-manifest SHA-256 is `22dd692577018e5d0f6848a516a8bad4fd2ed9b3c0a4dbb6594da00aa623b50b`.

The next evidence is an exact, explicitly non-poolable 27-cell validation over `DI-01`, `GR-01`, and `GR-03`, each repeated three times across `A_plain`, `B_direct`, and `C_skill`. It checks the repaired direct-caller, transitive-graph, and task-query-fitness paths before any full 165-cell rerun. Its scope SHA-256 is `9ca9ec5b0351d0c8bf57200dadd5e7ad689c3dd9ce69acc3c55145cf8ffbfac6` and its complete-run ceiling is 16,200 seconds. The Claude adapter remains the repeatedly debugged reference; only shared methodology corrections apply to both providers, and no Claude quality change is implied by this Codex recovery work.
