...
The current source relock is methodology SHA-256 `7e0b89b8c34b84f372d011ef392f469df02ca1b9c55ebdc01b0549cd6077c961`, machine-manifest SHA-256 `9c08713ca25900eecf9f397acf0629baf047c8f70b76502485f3e46dbdd2d80a`, and human-manifest SHA-256 `e4f4c9efb340f2daf545ed6973ca97395682ee385e2b9e9c909a284d1462c292`. The composed no-model selection dry run `bash benchmarks/run-all.sh codex --tasks=DI,GR --dry-run` passed with 10 tasks and 90 locked coordinates, a 54,000-second ceiling, and selection scope SHA-256 `c7264ecd554530a72ea19f5a57bcce272a0df19c47ec7f4db740a5f7ce3721ed`. These hashes bind the next user-launched run; they do not describe a paid result.

The first paid post-fix diagnostic attempt did not execute a model cell. Snapshot creation failed before model launch because macOS resolved the temporary root through the `/var` symlink; the strict permission-path guard rejected that alias. The second diagnostic attempt also produced zero model cells: `DI-01` intentionally stages `apply_func.py`, but the global clean-worktree admission rejected that legitimate pre-cell stage. The shared staged-change contract has now been replaced with `DiffImpactStageAdmission`: it records the exact Git porcelain status, repository commit, and SHA-256 of every intended modified file; the enclosing runtime lock separately verifies the Codemap index. It restores the target in `finally` and fails closed on extra tracked or untracked paths, same-file tampering, deletion, symlink/hardlink/type changes, Codemap-index drift, or commit drift. The dry-run now exercises the full staging lifecycle. Both failed runs are infrastructure diagnostics, not treatment evidence. The first correction still provides canonical default temporary-root handling, strict explicit/source symlink rejection, descriptor-identity snapshot copies, and boolean-only auth metadata. The full benchmark gate now passes with 1,606 tests plus 4 skips; the focused runner/launcher/manifest gate passes with 343 tests plus 2 skips; the active 90-coordinate selected-task no-model dry run is green.
...
The stopped partial artifact `benchmarks/results/codex-integration-20260802T095824Z` contains 55/165 rows: 18 complete A/B/C triplets, 15 structurally clean triplets, and 9 clean headline triplets. It is audit-only and non-poolable; no treatment effect is inferred from it. Root fixes in the relock include zero-argument `coupled` canonical detection, CQ-03 ranking by internal import count with complete ordered five `name + dep_count` rows, and RV-02 acceptance of natural `N modules directly import/depend` forms.

Verification is green: benchmarks 1,606 passed/4 skipped; Codemap 1,204 passed/11 skipped; focused runner/launcher/manifest tests 343 passed/2 skipped. Ruff, format, compile, Bash syntax, diff, and manifest checks pass; Ruff formatted 139 files, with intentional grammar fixtures excluded by project configuration. The active 90-coordinate selected-task no-model dry run passes. The benchmark source-builds the plugin package/install. Generic plugin-creator validation is a non-blocking residual because it does not accept the existing multi-runtime `codex-skills/` manifest layout/interface; no layout change is proposed.
...
Each cell receives an isolated `CODEX_HOME`. Permission profiles deny the copied credential, host agent roots, network, and source-tree writes; treatment arms may write only to the index-local coordination directory. The runner records answer, quality, extraction, treatment-adherence, Codemap-use, and transport failures per cell and continues after the admission smoke so these outcomes remain measurable. Terminal output uses `treatment:✓|✗` for the treatment contract and `codemap-used:✓|✗` for observed Codemap use: clean `A_plain` is `treatment:✓ codemap-used:✗`; a contaminated A row can be `treatment:✗ codemap-used:✓`; B/C require both treatment adherence and the required query evidence. The machine telemetry field names remain `treatment_adherence` and `compliance`.
...
The append-only `telemetry.jsonl` is the execution record. Rows retain `execution_index` and the actual randomized arm order so interrupted runs can be audited without rewriting history. The runner rejects existing raw/metadata artifacts for a new run; partial runs are audit-only and are never resumed, pooled, or re-scored as confirmatory evidence. A successful run also emits `telemetry-canonical.jsonl`, an atomically written derived view sorted by locked task position, repetition, and fixed treatment order. Human labels are `A_plain`, `B_direct`, and `C_skill`; machine telemetry and manifest IDs remain `A_plain`, `B_direct_required`, and `C_skill_required`. Terminal summaries and later paired analysis use the canonical view; raw and canonical files are never pooled or silently substituted. `run-metadata.json` records the canonical artifact status and SHA-256 alongside the raw telemetry hash.
...
The completed Codex run used the same frozen task objects, prompts, evaluators, target tree, and ground truth as the Claude structural benchmark. It is a descriptive integration result, not a pooled or confirmatory treatment claim.

| Field              | Locked value                                                                                                                                                                                                                                                                                                                        |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Artifact           | `benchmarks/results/codex-integration-20260802T135019Z` (local, ignored run output)                                                                                                                                                                                                                                                 |
| Target             | `pytorch-lightning` 2.6.5, commit `be98784a1a03581b7051a355ae1084fd352d7cea`                                                                                                                                                                                                                                                        |
| Software           | codemap-py 0.27.0; codex-rig 0.4.1; Codex CLI 0.146.0                                                                                                                                                                                                                                                                               |
| Model              | `gpt-5.6-luna`, high effort                                                                                                                                                                                                                                                                                                         |
| Design             | 55 tasks × 1 repetition × 3 treatments = 165 cells; 45 headline tasks + 10 diagnostics                                                                                                                                                                                                                                              |
| Run manifest       | SHA-256 `0cf2ecfb2c6e087c0cb75289b4b6abdf931634f8662cdde67f59dca78a85568b`                                                                                                                                                                                                                                                          |
| Artifact checksums | `run.log` `ad71f971dc65ffb9555f150321ae1ea6582a98e3c37dd32b7beb2653b3cfafdc`; raw telemetry `a3827cd484fb80ab129c1349092eb1be1de680b65b8902129c9b812ddbe87983`; canonical telemetry `d66226d8b2c0ae323470fc28ab06a7022d2d6090951ae3533ef9f8b3aac36495`; metadata `30c2735a455c4b3de482fa70ec156d2d6d30704b61517d6a55df4d2208bf61af` |

Headline descriptive results (continuous quality is the mean per-cell fitness; input and elapsed values are arithmetic means):

| Treatment  | Correct / 45 | Quality | Gross input | Elapsed | Adherence |
| ---------- | ------------ | ------- | ----------- | ------- | --------- |
| `A_plain`  | 38/45        | 0.955   | 169.8k      | 81.8s   | 45/45     |
| `B_direct` | 36/45        | 0.944   | 144.2k      | 55.6s   | 44/45     |
| `C_skill`  | 35/45        | 0.919   | 102.3k      | 52.4s   | 44/45     |

On the 42 clean common headline triplets, post-hoc 10,000-sample paired percentile-bootstrap intervals for geometric ratios (Codemap treatment / comparator) and mean quality differences were:

| Contrast | Gross input ratio    | Elapsed ratio        | Quality difference         |
| -------- | -------------------- | -------------------- | -------------------------- |
| B / A    | 0.752 [0.597, 0.938] | 0.775 [0.618, 0.961] | −0.0123 [−0.0660, +0.0429] |
| C / A    | 0.595 [0.492, 0.711] | 0.641 [0.522, 0.776] | −0.0223 [−0.0748, +0.0267] |
| C / B    | 0.791 [0.634, 0.978] | 0.827 [0.699, 0.993] | −0.0100 [−0.0488, +0.0238] |

**Validity and interpretation.** All 165 cells completed on 2026-08-02, but canonical pooling is ineligible: the run records two required-use misses and four extraction failures overall. It also has one repetition, cache/order exposure, and source-launch bytes that differ from the current manifest/runner. Therefore these results remain descriptive integration evidence only: they do not establish a causal Codemap advantage, confirmatory effect, or cross-provider raw-token comparison. The lower B/C input and elapsed means are encouraging, but quality is lower for both Codemap treatments and every quality interval includes zero; post-fix quality is not yet demonstrated.

The run exposed deterministic integration defects rather than one single model failure: Skill routing sent `central`/`fn-blast` work toward the wrong query family; the static graph did not preserve alias/re-export completeness; evaluator parsing and DiffImpact staging diverged from the shared contract; and one repetition left substantial order/cache variance. The recovery is now implemented in the source surfaces: schema 12 records alias/re-export limitations, bounded compact output reports limitation counts with bounded samples, Claude/Codex Skill routing is mirrored with recovery guidance, evaluator and staging logic are shared, required Codemap queries are admitted semantically, and provenance/index checks fail closed. DiffImpact staging now has an explicit admission object with exact Git status, intended-file SHA-256, and repository-commit checks plus strict restoration; the enclosing runtime lock separately verifies the Codemap index. The intentional `DI-01` stage is therefore exercised by the selected-task dry-run rather than blocked by global clean-worktree admission. The historical `bash benchmarks/run-all.sh codex --diagnostic --dry-run` is retained only as superseded evidence; the active equivalent is `bash benchmarks/run-all.sh codex --tasks=DI,GR --dry-run`, which passed 10 tasks and 90 coordinates without a model or authentication source and emits one legend for the top-level run. The benchmark suite passed 1,606 tests with 4 skipped; the focused runner/launcher/manifest suite passed 343 with 2 skipped; the plugin suite passed 1,204 with 11 skipped. The active Codex machine-manifest SHA-256 is `9c08713ca25900eecf9f397acf0629baf047c8f70b76502485f3e46dbdd2d80a`. Paid post-fix quality remains unknown and requires fresh manifest review and authorization; this does not retroactively repair either failed diagnostic.

The next evidence is an exact, explicitly non-poolable 54-cell diagnostic: `DI-01`, `DI-05`, `DI-06`, `GR-01`, `GR-03`, and `GR-04`, each repeated three times across `A_plain`, `B_direct`, and `C_skill`. It requires separate review of the regenerated manifest and dry-run before one fresh paid authorization. A diagnostic can validate recovery and locate residual regressions, but cannot satisfy confirmatory acceptance. The Claude adapter remains the repeatedly debugged reference; only shared methodology corrections apply to both providers, and no Claude quality change is implied by this Codex recovery work.

The run is retained for audit and follow-up, not silently mixed with Claude results. Audit the local artifacts with `benchmarks/results/codex-integration-20260802T135019Z/checksums.sha256`; the ignored result directory is intentionally not a published fixture. Exact reproduction additionally requires recovery of the launch manifest and runner bytes recorded by the run because the current files have different hashes.
